<template>
    <div class="p-6 bg-gray-50 min-h-screen">
      ...
      <div class="bg-white p-6 rounded-lg border relative">
          ...
          <tr v-for="(row, index) in groups" :key="index" class="hover:bg-gray-50">
            ...
            <td class="border px-4 py-2 text-center">
              ...
              <Button
                    icon="pi pi-users"
                    class="p-button-text text-blue-500"
                    @click="openAssignUserDrawer(row)"
                    label="Assign User"
                  />
              <Button
                    icon="pi pi-plus-circle"
                    class="p-button-text text-blue-500"
                    @click="openAssignRoleDrawer(row)"
                    label="Assign Role"
                  />
            </td>
          </tr>
        ...
        <!-- Create Modal -->
        <Drawer v-if="openAssignUserDrawer" v-model:visible="showAssignUserDrawer"  header="List of Operators" position="right">
        ...
        <Drawer v-if="openAssignRoleDrawer" v-model:visible="showAssignRoleDrawer"  header="List of Roles" position="right">
          ...
          <div class="bg-white p-6 rounded-lg shadow-lg w-3/4 md:w-1/2 max-h-screen overflow-y-auto">
            ...
            <form @submit.prevent="createGroup" class="space-y-4">
              <div>
                ...
                <input v-model="newGroup.name" type="text" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring focus:ring-blue-300" />
          ...
          <div class="bg-white p-6 rounded-lg shadow-lg w-3/4 md:w-1/2">
            ...
            <form @submit.prevent="updateGroup" class="space-y-4">
              <div>
                ...
                <input v-model="editingGroup.name" type="text" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring focus:ring-blue-300" />
                <span v-if="errors.name" class="text-red-500 text-sm">{{ errors.name }}</span>
              </div>
              <div class="flex justify-end">
                <button type="button" @click="closeModal" class="mr-2 bg-gray-600 text-white py-2 px-4 rounded-lg hover:bg-gray-700 transition duration-200">Cancel</button>
