<template>
  ...
  try {
    ...
    if (response.data.token) {
      ...
      const authme = await axios.get("/api/auth/me", { headers });
      ...
      // Retrieve and store route permissions
      ...
      // Handle currency selection and store in localStorage
      const selectedCurrency = localStorage.getItem('selectedCurrency')
        || (JSON.parse(localStorage.getItem("userCurrency"))?.[0]?.CurrencyId);
      ...
      const totalCount = await axios.get("/api/users/totalCount", { headers });

      if (totalCount?.data?.totalList) {
      ...
      // Now stop the loading spinner and navigate to the dashboard
      this.loading = false;

      // Navigate to the dashboard view using Vue Router
      this.$router.push("/dashboard");  // Vue Router handles the route transition to the dashboard view
    } else {
      this.loading = false;
